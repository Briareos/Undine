...
var GuzzleServer = function (port, log) {
    ...
    var receivedRequest = function (request, req, res) {
        if (req.url.indexOf("/guzzle-server") === 0) {
            ...
            var duration = req.headers && req.headers['duration'] ? parseInt(req.headers['duration'], 10) : 0;
            if (util.isArray(response.headers)) {
                response.headers = {};
            }
            response.headers['requested-at'] = new Date().getTime() / 1000;
            setTimeout(function () {
                response.headers['responded-at'] = new Date().getTime() / 1000;
                res.writeHead(response.statusCode, response.reasonPhrase, response.headers);
                res.end(response.body);
                console.log('Sending response...', new Date().getTime());
            }, duration);
            that.requests.push(request);
            console.log('Received request...', new Date().getTime());
        }
    };
