...
/**
 * Pretty much a copy of HttpKernelTest, with updated instantiation and namespace.
 * Our async kernel must be 100% compatible.
 *
 * To update this file:
 *  - copy the HttpKernelTest
 *  - replace "new HttpKernel" with "new AsyncHttpKernel"
 *  - replace the namespace "Symfony\Component\HttpKernel\Tests\" with "Undine\Tests\Http\"
 */
class AsyncHttpKernelTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        $kernel = new AsyncHttpKernel(new EventDispatcher(), $this->getResolver(function () { throw new \RuntimeException(); }));
    ...
    public function testHandleWhenControllerThrowsAnExceptionAndCatchIsTrueWithAHandlingListener()
    {
        ...
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () { throw new \RuntimeException('foo'); }));
    ...
    public function testHandleWhenControllerThrowsAnExceptionAndCatchIsTrueWithANonHandlingListener()
    {
        ...
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () use ($exception) { throw $exception; }));
    ...
    public function testHandleExceptionWithARedirectionResponse()
    {
        ...
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () { throw new AccessDeniedHttpException(); }));
    ...
    public function testHandleHttpException()
    {
        ...
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () { throw new MethodNotAllowedHttpException(array('POST')); }));
        $response = $kernel->handle(new Request());
    ...
    public function testHandleWhenAnExceptionIsHandledWithASpecificStatusCode($responseStatusCode, $expectedStatusCode)
    {
        ...
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () { throw new \RuntimeException(); }));
        $response = $kernel->handle(new Request());
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(false));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver('foobar'));
    ...
    public function testHandleWhenTheControllerIsAClosure()
    {
        ...
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () use ($response) { return $response; }));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(new Controller()));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver('Undine\Tests\Http\controller_func'));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(array(new Controller(), 'controller')));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(array('Undine\Tests\Http\Controller', 'staticcontroller')));
    ...
    {
        $dispatcher = new EventDispatcher();
        $kernel = new AsyncHttpKernel($dispatcher, $this->getResolver(function () { return 'foo'; }));
